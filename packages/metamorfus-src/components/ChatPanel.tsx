...
import { Send, TerminalSquare, User, Bot, Loader2, Eye } from 'lucide-react';
import { SystemConfig, MindState } from '../types';

interface Message {
  role: 'user' | 'assistant';
  content: string;
  /** Optional vision attachment shown as a thumbnail above the bubble. */
  imageUrl?: string;
}
...
export default function ChatPanel({ config, mindState }: ChatPanelProps) {
  ...
  const handleVisionDescribe = async () => {
    if (!visionUrl.trim() || isLoading) return;
    const url = visionUrl.trim();
    setIsLoading(true);
    setMessages(prev => [...prev, {
      role: 'user',
      content: `Describe this image: ${url}`,
      imageUrl: url,
    }]);
    setVisionUrl('');
    setVisionOpen(false);
    try {
      ...
      if (!resp.ok) {
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: `Vision skill failed: ${data.error ?? `HTTP ${resp.status}`}`,
        }]);
        return;
      }
      const header = data.reasoning
        ? `[vision_describe_protocol — ${data.model}]\nReasoning: ${data.reasoning}\n\n`
        : `[vision_describe_protocol — ${data.model}]\n\n`;
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: header + (data.description ?? 'No description returned.'),
      }]);
    } catch (e: any) {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `Vision transport error: ${e?.message ?? 'unknown'}`,
      }]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-full bg-odc-bg text-odc-text">
                 ...
                 <div className={`p-3 rounded max-w-[80%] ${msg.role === 'user' ? 'bg-blue-900/40 border border-blue-500/30 text-blue-100' : 'bg-white/5 border border-white/10 text-gray-300'}`}>
                     {msg.imageUrl && (
                       <img
                         src={msg.imageUrl}
                         alt={msg.content}
                         className="mb-2 max-h-48 rounded border border-white/10"
                       />
                     )}
       ...
       <div className="p-4 border-t border-white/10 bg-odc-panel mt-auto">
           {visionOpen && (
             <div className="mb-2 flex gap-2 bg-black/40 border border-white/10 rounded p-2">
               <input
                 type="url"
                 value={visionUrl}
                 onChange={e => setVisionUrl(e.target.value)}
                 onKeyDown={e => e.key === 'Enter' && handleVisionDescribe()}
                 placeholder="https://… image URL to describe via vision_describe_protocol"
                 disabled={isLoading}
                 className="flex-1 bg-transparent text-sm text-white placeholder-gray-500 font-mono focus:outline-none"
               />
               <button
                 onClick={handleVisionDescribe}
                 disabled={isLoading || !visionUrl.trim()}
                 className="px-2 py-1 text-xs rounded bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-500/30 disabled:opacity-50"
               >
                 DESCRIBE
               </button>
               <button
                 onClick={() => { setVisionOpen(false); setVisionUrl(''); }}
           ...
           <div className="relative flex items-center gap-2">
               <button
                 ...
                 disabled={isLoading}
                 title="Describe an image (vision_describe_protocol)"
                 className={`p-2 rounded border transition-colors ${visionOpen ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40' : 'bg-white/5 text-odc-muted border-white/10 hover:text-white'} disabled:opacity-50`}
               >
                 <Eye size={16} />
               </button>
               <input 
                   ...
                   disabled={isLoading}
                   className="flex-1 bg-black/40 border border-white/10 rounded px-4 py-3 text-sm focus:outline-none focus:border-odc-accent text-white placeholder-gray-500 font-mono transition-colors disabled:opacity-50 pr-12"
               />
               <button 
                  onClick={handleSend}
